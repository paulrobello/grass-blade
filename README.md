...
- `?contract=meadow-delivery`, `?contract=flower-sweep`, `?contract=woodland-cleanup`, `?contract=timed-harvest`, or `?contract=field-sprint` selects the authored contract template; unknown IDs fall back to `meadow-delivery`.
