...
Phase 4 presentation/accessibility systems are implemented and verified. A code-native onboarding card now appears first with a focused `Start Cutting` button; movement, pause, fullscreen, and simulation time stay inactive until that explicit Start action is activated. After Start, focus moves to the canvas and the normal keyboard/touch contract flow resumes. Touch and pen drags now show a temporary cyan stick at the drag origin so mobile movement direction is visible without adding a permanent control pad. Pause and results overlays move focus to their primary visible action, active contracts automatically pause and release held input when the browser window blurs, and Space is suppressed as a browser-scroll key while the canvas owns input. High-contrast HUD and dialog styling is available through `?contrast=high` and also activates from `forced-colors: active` or `prefers-contrast: more`. Reduced-motion mode is honored from `prefers-reduced-motion` and can be forced with `?motion=reduced` or disabled for testing with `?motion=standard`. Procedural WebAudio feedback starts only after the Start gesture and includes a blade RPM hum, resource-matched cut sounds, level-up stingers, completion audio, a mute shortcut, and independent Master/Music/Effects controls. `make accessibility-check` runs the current Phase 4 browser accessibility verifier for focus/no-scroll, blur-pause freeze, reduced-motion, and 200%-equivalent grayscale readability. `make playthrough-check-headed` runs a visible-browser contract pass covering movement, fullscreen exit-before-pause behavior, pause/resume focus, durable-target reward feedback, and completion.

Phase 5 expansion has authored contract variants and a Start-screen contract chooser. The default `meadow-delivery` contract remains the balanced `50/10/6/6` quota set, but now uses a broad non-square starter meadow with a main route, side routes, rounded clearings, and carved gaps instead of a full square lawn. Choose Flower Sweep on the Start screen, or add `?contract=flower-sweep`, to play a flower-drift contract with lighter Grass, Fiber, and Wood quotas plus every authored Flower target (`34/320/4/4`) in an asymmetric branching corridor-shaped growth arena. Choose Woodland Cleanup, or add `?contract=woodland-cleanup`, to emphasize weeds and saplings with heavier Fiber and Wood quotas (`30/6/8/8`) in connected woodland clearings. Choose Timber Trail, or add `?contract=timber-trail`, to play a 90-second grove route with `250 Grass / 260 Flowers / 28 Fiber / 28 Wood` quotas that require all dense weeds, all shrubs, all five saplings, and three mature trees in deterministic balance. Choose Rock Garden, or add `?contract=rock-garden`, to play a 70-second obstacle slalom with visible rock cutouts and `190 Grass / 220 Flowers / 16 Fiber / 10 Wood` quotas that require sustained soft-target clearing plus all saplings. Choose Hedge Maze, or add `?contract=hedge-maze`, to play an 80-second shrub route with `183 Grass / 300 Flowers / 28 Fiber` quotas that require all dense weeds and shrubs. Choose Timed Harvest, or add `?contract=timed-harvest`, to play a 65-second loop-route challenge with nearly full `170 Grass / 300 Flowers / 18 Fiber` quotas and no Wood requirement. Choose Field Sprint, or add `?contract=field-sprint`, to play a 50-second flower-lane sprint with heavier `175 Grass / 230 Flowers` quotas and no durable-target requirements. Choose Weed Rush, or add `?contract=weed-rush`, to play a 55-second switchback route with `150 Grass / 220 Flowers / 18 Fiber` quotas and no Wood requirement. Choose Reed Run, or add `?contract=reed-run`, to play a 60-second golden-reed loop with `205 Grass / 270 Flowers / 10 Fiber` quotas that explicitly route the deterministic balance path through all ten Fiber Reeds. Choose Clover Circuit, or add `?contract=clover-circuit`, to play a 75-second figure-eight route with `240 Grass / 320 Flowers / 28 Fiber` quotas, every flower pocket, and every Fiber target but no Wood gate. Choose Orchard Loop, or add `?contract=orchard-loop`, to play an 85-second wood circuit with `240 Grass / 260 Flowers / 20 Fiber / 22 Wood` quotas that require all five saplings, two mature trees, every dense weed, and four shrubs. Choose Brook Bend, or add `?contract=brook-bend`, to play a 65-second S-bend route with `200 Grass / 250 Flowers / 24 Fiber` quotas that require all dense weeds and six shrubs while avoiding a Wood gate. Choose Harvest Spiral, or add `?contract=harvest-spiral`, to play an 80-second tightening spiral route with `240 Grass / 260 Flowers / 24 Fiber / 22 Wood` quotas that require six shrubs, all five saplings, and two mature trees. Choose Crescent Grove, or add `?contract=crescent-grove`, to play a 75-second crescent route with `220 Grass / 240 Flowers / 20 Fiber / 16 Wood` quotas that require all dense weeds, four shrubs, all five saplings, and one mature tree. Choose Forked Thicket, or add `?contract=forked-thicket`, to play an 85-second three-way branch route with `260 Grass / 280 Flowers / 24 Fiber / 22 Wood` quotas that require all dense weeds, six shrubs, all five saplings, and two mature trees. Choose Switchback Orchard, or add `?contract=switchback-orchard`, to play a 90-second zig-zag orchard route with `270 Grass / 300 Flowers / 28 Fiber / 28 Wood` quotas that require all dense weeds, all shrubs, all five saplings, and three mature trees. Choose Braided Meadow, or add `?contract=braided-meadow`, to play a 70-second woven flower-lane route with `230 Grass / 300 Flowers / 20 Fiber` quotas that require all dense weeds and four shrubs without a Wood gate. Choose Willow Weave, or add `?contract=willow-weave`, to play a 105-second woven grove route with `250 Grass / 300 Flowers / 24 Fiber / 22 Wood` quotas that require all dense weeds, six shrubs, all five saplings, and two mature trees. Choose Petal Gate, or add `?contract=petal-gate`, to play an 88-second flower-gate route with `250 Grass / 320 Flowers / 28 Fiber` quotas that require every flower pocket plus every dense weed and shrub without a Wood gate. Choose Sunset Switchback, or add `?contract=sunset-switchback`, to play a 160-second diagonal route with `260 Grass / 300 Flowers / 28 Fiber` quotas that require all dense weeds and shrubs in a zig-zag flower route without a Wood gate. Choose Cedar Crossroads, or add `?contract=cedar-crossroads`, to play an 86-second four-way timber route with `260 Grass / 260 Flowers / 24 Fiber / 22 Wood` quotas that require all dense weeds, six shrubs, all saplings, and two mature trees. Choose Lagoon Braid, or add `?contract=lagoon-braid`, to play a 70-second braided lagoon route with `240 Grass / 300 Flowers / 24 Fiber` quotas that require all dense weeds and six shrubs without a Wood gate. Choose Wildflower Narrows, or add `?contract=wildflower-narrows`, to play a 70-second narrow bloom route with `230 Grass / 320 Flowers / 24 Fiber` quotas that require every flower pocket, all dense weeds, and six shrubs without a Wood gate. Choose Daisy Drift, or add `?contract=daisy-drift`, to play a 76-second drifting bloom route with `245 Grass / 320 Flowers / 24 Fiber` quotas that require every flower pocket, all dense weeds, and six shrubs without a Wood gate. Choose Clear Every Patch, or add `?contract=clear-every-patch`, to play a split-clearings full-clean objective that requires every generated Grass and Flower patch to be cut before completion. Timed contracts show a visible pre-start time badge, a countdown HUD, low-time and critical-time dial states for the final ten and five seconds, and a `Time Up` result if the timer expires before every quota is complete. Chooser filter buttons now show compact bucket counts, and cards include compact pace, focus, difficulty, and Gold target-time details so the larger contract list is easier to scan on mobile. Successful completions update a local best time per authored contract, shown on the chooser and result card with earned Gold/Silver/Bronze medals; the chooser also summarizes aggregate medal progress across all authored contracts. Timed-out attempts do not overwrite records. Zero-quota resources are omitted from the chooser quota copy and active HUD row. Flower drifts are now split into twenty smaller authoritative flower pockets with tighter cut radii, so touching the edge of a patch no longer cuts, topples, or completes the entire patch. Authored growth silhouettes also place subtle deterministic low stones/moss scallops along route edges so paths and clearings read as intentional spaces instead of invisible square masks, while actual movement blocking comes only from visible solid targets and the outer world bounds. Unknown contract IDs safely fall back to Meadow Delivery.

Ring Grove is available with `?contract=ring-grove`: it is an 80-second looped grove route with `245 Grass / 260 Flowers / 24 Fiber / 16 Wood` quotas, a bare center clearing, and deterministic balance that requires all dense weeds, six shrubs, all five saplings, and one mature tree.

Twin Glade is available with `?contract=twin-glade`: it is a 75-second twin-clearing route with `230 Grass / 260 Flowers / 20 Fiber / 10 Wood` quotas, mirrored flower glades, narrow connecting lanes, and deterministic balance that requires all dense weeds, four shrubs, and all five saplings.
...
Stone Bloom is available with `?contract=stone-bloom`: it is a 68-second rock-chicane flower route with `240 Grass / 300 Flowers / 24 Fiber` quotas, visible stone blockers carving the route, and deterministic balance that requires most flower targets, all dense weeds, and six shrubs without a Wood gate.

Willow Weave is available with `?contract=willow-weave`: it is a 105-second woven grove route with `250 Grass / 300 Flowers / 24 Fiber / 22 Wood` quotas, braided flower lanes through saplings and mature-tree pockets, and deterministic balance that requires all dense weeds, six shrubs, all five saplings, and two mature trees.

Petal Gate is available with `?contract=petal-gate`: it is an 88-second flower-gate route with `250 Grass / 320 Flowers / 28 Fiber` quotas, no Wood gate, a lattice of cross-lanes and compact internal gaps, and deterministic balance that requires every flower pocket, all dense weeds, and all shrubs.

Sunset Switchback is available with `?contract=sunset-switchback`: it is a 160-second diagonal switchback route with `260 Grass / 300 Flowers / 28 Fiber` quotas, no Wood gate, sunset-toned flower lanes, and deterministic balance that requires all dense weeds and all shrubs.

Cedar Crossroads is available with `?contract=cedar-crossroads`: it is an 86-second four-way timber route with `260 Grass / 260 Flowers / 24 Fiber / 22 Wood` quotas, cedar-lane crossings, flower pockets, and deterministic balance that requires all dense weeds, six shrubs, all five saplings, and two mature trees.

Lagoon Braid is available with `?contract=lagoon-braid`: it is a 70-second braided lagoon route with `240 Grass / 300 Flowers / 24 Fiber` quotas, alternating flower banks, internal lagoon-like no-growth pockets, and deterministic balance that requires all dense weeds and six shrubs without a Wood gate.

Wildflower Narrows is available with `?contract=wildflower-narrows`: it is a 70-second narrow bloom route with `230 Grass / 320 Flowers / 24 Fiber` quotas, tight flower corridors, compact internal gaps, and deterministic balance that requires every flower pocket, all dense weeds, and six shrubs without a Wood gate.
...
The current default visual field keeps a deterministic 104 by 104 grass-tuft index space for masks and chunks, with 7,520 visible instanced fourteen-blade grass tufts—105,280 narrow blades before distance LOD—inside the authored starter-meadow silhouette. It is split across 8 by 8 frustum-cullable render chunks, plus 880 low-poly flowers arranged across 16 seeded drifts, 108 broad-leaf weed instances mapped nine-to-one onto twelve targets, 70 golden reed instances mapped seven-to-one onto ten Fiber Reed targets, five layered-foliage saplings, eight mature trees, and 83-196 low-profile arena-edge marker instances depending on the selected contract shape. Default near grass keeps fourteen blades per tuft, while far visible chunks switch to eight blades per tuft; low quality keeps the same authored coverage and target layout but disables antialiasing and shadow rendering, renders eight blades per visible tuft—60,160 narrow blades before distance LOD—and remains above the PRD's lower-quality 45 blades per world unit squared threshold. Each visible 4 by 4 group of grass visuals maps to one of 486 logical reward targets, but a finer persistent visual mask flattens only the individual tufts intersected by the blade's swept path. Flowers, weeds, and reeds map to stable logical targets, while sapling and tree visuals and collision share the same authoritative world targets. Grass, flower, weed, and reed source silhouettes use renderer-owned, root-anchored fall lifecycles before resolving to persistent stubble, clippings, stems, petals, or leaf litter. Cut vegetation then remains as stubble, clippings, stems, petals, leaf litter, or a stump for the rest of the run. A fixed 240-slot instanced fragment pool renders short-lived secondary cut bursts without allocating per frame, and reduced-motion mode keeps the causal fall while shortening both vegetation and fragment lifetimes. `window.render_game_to_text()` exposes a deterministic meadow density report, quality settings, grass chunk visibility/distance-LOD diagnostics, GPU cut-mask diagnostics, arena boundary marker counts, and recent frame timing so the grass coverage, grass-blade density, flower-drift coverage, blossom density, active decorative-grass chunk count, current visible grass blade budget, cut-mask alignment, active boundary-marker count, and active render-cost preset can be checked against the Phase 3 renderer thresholds.
