...
- `?contract=meadow-delivery`, `?contract=flower-sweep`, `?contract=woodland-cleanup`, `?contract=timber-trail`, `?contract=rock-garden`, `?contract=hedge-maze`, `?contract=timed-harvest`, `?contract=field-sprint`, `?contract=weed-rush`, `?contract=clover-circuit`, `?contract=orchard-loop`, `?contract=brook-bend`, `?contract=harvest-spiral`, `?contract=crescent-grove`, `?contract=forked-thicket`, `?contract=switchback-orchard`, or `?contract=clear-every-patch` selects the authored contract template; unknown IDs fall back to `meadow-delivery`.
