...
describe("active game state", () => {
  ...
  it("creates and completes the authored Reed Run contract before the clock expires", () => {
    const state = createInitialState(12345, "reed-run");
    ...
    expect(state.objectives.grass.target).toBe(205);
    expect(state.objectives.flowers.target).toBe(270);
    expect(state.objectives.fiber.target).toBe(10);
    expect(state.objectives.wood.target).toBe(0);
    expect(state.targets.filter((target) => target.kind === "fiberReed")).toHaveLength(
      FIBER_REED_COUNT,
    );
    ...
    expect(state.mode).toBe("complete");
    expect(state.elapsedSeconds).toBeLessThan(60);
    expect(state.inventory).toEqual({ grass: 205, flowers: 270, fiber: 10, wood: 0 });
    expect(state.result).toMatchObject({
      status: "complete",
      timeLimitSeconds: 60,
      cutTargets: 485,
      highestLevel: 8,
      finalInventory: { grass: 205, flowers: 270, fiber: 10, wood: 0 },
      completionRevision: 485,
    });
  });
  ...
  it("creates and completes the authored Brook Bend contract before the clock expires", () => {
    const state = createInitialState(12345, "brook-bend");
    ...
    expect(state.objectives.flowers.target).toBe(250);
  ...
  it("creates and completes the authored Forked Thicket contract before the clock expires", () => {
    const state = createInitialState(12345, "forked-thicket");
    ...
    expect(state.mode).toBe("complete");
    expect(state.elapsedSeconds).toBeLessThan(85);
    expect(state.inventory).toEqual({ grass: 260, flowers: 280, fiber: 24, wood: 22 });
    expect(state.result).toMatchObject({
      status: "complete",
      timeLimitSeconds: 85,
      cutTargets: 565,
      highestLevel: 8,
      finalInventory: { grass: 260, flowers: 280, fiber: 24, wood: 22 },
      completionRevision: 565,
    });
    expect(state.targets.filter((target) => target.kind === "denseWeed")).toHaveLength(12);
    expect(state.targets.filter((target) => target.kind === "shrub")).toHaveLength(6);
    expect(state.targets.filter((target) => target.kind === "sapling")).toHaveLength(5);
  ...
  it("creates and completes the authored Switchback Orchard contract before the clock expires", () => {
    const state = createInitialState(12345, "switchback-orchard");
    ...
    expect(state.objectives.grass.target).toBe(270);
    expect(state.objectives.flowers.target).toBe(300);
    expect(state.objectives.fiber.target).toBe(28);
    expect(state.objectives.wood.target).toBe(28);
    ...
    expect(state.mode).toBe("complete");
    expect(state.elapsedSeconds).toBeLessThan(90);
    expect(state.inventory).toEqual({ grass: 270, flowers: 300, fiber: 28, wood: 28 });
    expect(state.result).toMatchObject({
      status: "complete",
      timeLimitSeconds: 90,
      cutTargets: 598,
      highestLevel: 8,
      finalInventory: { grass: 270, flowers: 300, fiber: 28, wood: 28 },
      completionRevision: 598,
    });
    expect(state.targets.filter((target) => target.kind === "denseWeed")).toHaveLength(12);
    expect(state.targets.filter((target) => target.kind === "shrub")).toHaveLength(8);
    expect(state.targets.filter((target) => target.kind === "sapling")).toHaveLength(5);
  ...
  it("creates and completes the authored Braided Meadow contract before the clock expires", () => {
    const state = createInitialState(12345, "braided-meadow");
  ...
  it("creates and completes the authored Twin Glade contract before the clock expires", () => {
    const state = createInitialState(12345, "twin-glade");
  ...
  it("creates and completes the authored Frost Ribbons contract before the clock expires", () => {
    const state = createInitialState(12345, "frost-ribbons");
    ...
    expect(state.mode).toBe("complete");
    expect(state.elapsedSeconds).toBeLessThan(65);
    expect(state.inventory).toEqual({ grass: 220, flowers: 280, fiber: 18, wood: 0 });
    expect(state.result).toMatchObject({
      status: "complete",
      timeLimitSeconds: 65,
      cutTargets: 515,
      highestLevel: 8,
      finalInventory: { grass: 220, flowers: 280, fiber: 18, wood: 0 },
      completionRevision: 515,
    });
    expect(state.targets.filter((target) => target.kind === "denseWeed")).toHaveLength(12);
  ...
  it("creates and completes the authored Prism Prairie contract before the clock expires", () => {
    const state = createInitialState(12345, "prism-prairie");
    ...
    expect(state.mode).toBe("complete");
    expect(state.elapsedSeconds).toBeLessThan(70);
    expect(state.inventory).toEqual({ grass: 235, flowers: 320, fiber: 24, wood: 0 });
    expect(state.result).toMatchObject({
      status: "complete",
      timeLimitSeconds: 70,
      cutTargets: 573,
      highestLevel: 8,
      finalInventory: { grass: 235, flowers: 320, fiber: 24, wood: 0 },
      completionRevision: 573,
    });
    expect(state.targets.filter((target) => target.kind === "denseWeed")).toHaveLength(12);
    expect(state.targets.filter((target) => target.kind === "shrub")).toHaveLength(6);
    expect(state.targets.filter((target) => target.kind === "sapling")).toHaveLength(0);
  ...
  it("creates and completes the authored Willow Weave contract before the clock expires", () => {
    const state = createInitialState(12345, "willow-weave");
  ...
  it("creates and completes the authored Petal Gate contract before the clock expires", () => {
    const state = createInitialState(12345, "petal-gate");
  ...
  it("creates and completes the authored Sunset Switchback contract before the clock expires", () => {
    const state = createInitialState(12345, "sunset-switchback");

    expect(state.contract).toEqual({
      id: "sunset-switchback",
      title: "Sunset Switchback",
      summary: "A 140-second diagonal route through sunset flower lanes and Fiber hedges.",
    ...
    expect(state.mode).toBe("complete");
    expect(state.elapsedSeconds).toBeLessThan(140);
  ...
  it("creates and completes the authored Lagoon Braid contract before the clock expires", () => {
    const state = createInitialState(12345, "lagoon-braid");
  ...
  it("creates and completes the authored Wildflower Narrows contract before the clock expires", () => {
    const state = createInitialState(12345, "wildflower-narrows");
  ...
  it("creates and completes the authored Berry Bloom contract by cutting soft crops", () => {
    const state = createInitialState(12345, "berry-bloom");
    ...
    expect(state.objectives.grass.target).toBe(240);
    expect(state.objectives.flowers.target).toBe(FLOWER_TARGET_COUNT + 20);
    expect(state.objectives.fiber.target).toBe(28);
    expect(state.objectives.wood.target).toBe(0);
    ...
    expect(state.inventory).toEqual({
      grass: 240,
      flowers: FLOWER_TARGET_COUNT + 20,
      fiber: 28,
      wood: 0,
    });
    expect(state.result).toMatchObject({
    ...
    expect(state.targets.filter((target) => target.kind === "softCrop")).toHaveLength(10);
    expect(state.targets.filter((target) => target.kind === "denseWeed")).toHaveLength(12);
    expect(state.targets.filter((target) => target.kind === "shrub")).toHaveLength(8);
    expect(state.targets.filter((target) => target.kind === "sapling")).toHaveLength(0);
    expect(state.targets.filter((target) => target.kind === "matureTree")).toHaveLength(0);
  ...
  it("creates and completes the authored Daisy Drift contract before the clock expires", () => {
    ...
    expect(state.mode).toBe("complete");
    expect(state.elapsedSeconds).toBeLessThan(76);
    expect(state.inventory).toEqual({
      grass: 245,
      flowers: FLOWER_TARGET_COUNT,
      fiber: 24,
  ...
  it("builds contract-specific non-square growth arenas", () => {
    ...
    const reedRun = createMeadowLayout(12345, "reed-run");
    ...
    expect(reedRun.arenaShape).toBe("reed-run");
    ...
    expect(reedRun.flowerTargets).toHaveLength(FLOWER_TARGET_COUNT);
    ...
    expect(forkedThicket.flowerTargets).toHaveLength(FLOWER_TARGET_COUNT);
    expect(switchbackOrchard.flowerTargets).toHaveLength(FLOWER_TARGET_COUNT);
    expect(braidedMeadow.flowerTargets).toHaveLength(FLOWER_TARGET_COUNT);
    ...
    expect(reedRun.boundaryMarkers.length).toBeGreaterThan(110);
    ...
    expect(forkedThicket.boundaryMarkers.length).toBeGreaterThan(120);
    expect(switchbackOrchard.boundaryMarkers.length).toBeGreaterThan(120);
    expect(braidedMeadow.boundaryMarkers.length).toBeGreaterThan(130);
    ...
    expect(reedRun.grassVisuals).toHaveLength(GRASS_VISUAL_COLUMNS * GRASS_VISUAL_COLUMNS);
    ...
    expect(forkedThicket.grassVisuals).toHaveLength(GRASS_VISUAL_COLUMNS * GRASS_VISUAL_COLUMNS);
    expect(switchbackOrchard.grassVisuals).toHaveLength(
      GRASS_VISUAL_COLUMNS * GRASS_VISUAL_COLUMNS,
    );
    expect(braidedMeadow.grassVisuals).toHaveLength(GRASS_VISUAL_COLUMNS * GRASS_VISUAL_COLUMNS);
    ...
    expect(lagoonBraid.grassVisuals).toHaveLength(GRASS_VISUAL_COLUMNS * GRASS_VISUAL_COLUMNS);
    expect(wildflowerNarrows.grassVisuals).toHaveLength(
      GRASS_VISUAL_COLUMNS * GRASS_VISUAL_COLUMNS,
    );
    ...
    expect(reedRun.grassCells.length).toBeGreaterThan(205);
    ...
    expect(forkedThicket.grassCells.length).toBeGreaterThan(260);
    expect(switchbackOrchard.grassCells.length).toBeGreaterThan(270);
    expect(braidedMeadow.grassCells.length).toBeGreaterThan(230);
    ...
    expect(lagoonBraid.grassCells.length).toBeGreaterThan(240);
    ...
    expect(forkedThicket.grassCells.length).toBeLessThan(meadow.grassCells.length * 0.64);
    expect(switchbackOrchard.grassCells.length).toBeLessThan(meadow.grassCells.length * 0.74);
    expect(braidedMeadow.grassCells.length).toBeLessThan(meadow.grassCells.length * 0.79);
    ...
    expect(countVisibleGrassVisuals(flowerSweep)).toBeLessThan(
      GRASS_VISUAL_COLUMNS * GRASS_VISUAL_COLUMNS * 0.75,
    );
    ...
    expect(countVisibleGrassVisuals(reedRun)).toBeLessThan(
      GRASS_VISUAL_COLUMNS * GRASS_VISUAL_COLUMNS * 0.58,
    );
    expect(countVisibleGrassVisuals(twinGlade)).toBeLessThan(
      GRASS_VISUAL_COLUMNS * GRASS_VISUAL_COLUMNS * 0.62,
    );
    expect(countVisibleGrassVisuals(frostRibbons)).toBeLessThan(
      GRASS_VISUAL_COLUMNS * GRASS_VISUAL_COLUMNS * 0.7,
    );
    ...
    expect(countVisibleGrassVisuals(willowWeave)).toBeLessThan(
      GRASS_VISUAL_COLUMNS * GRASS_VISUAL_COLUMNS * 0.72,
    );
    expect(countVisibleGrassVisuals(petalGate)).toBeLessThan(
      GRASS_VISUAL_COLUMNS * GRASS_VISUAL_COLUMNS * 0.72,
    );
    ...
    expect(countVisibleGrassVisuals(lagoonBraid)).toBeLessThan(
      GRASS_VISUAL_COLUMNS * GRASS_VISUAL_COLUMNS * 0.78,
    );
    expect(countVisibleGrassVisuals(wildflowerNarrows)).toBeLessThan(
      GRASS_VISUAL_COLUMNS * GRASS_VISUAL_COLUMNS * 0.72,
    );
    ...
    const reedRunState = createInitialState(12345, "reed-run");
    ...
    expect(reedRunState.targets.filter((target) => target.kind === "grass")).toHaveLength(
      reedRun.grassCells.length,
    );
    ...
    const brookBendState = createInitialState(12345, "brook-bend");
    expect(brookBendState.targets.filter((target) => target.kind === "grass")).toHaveLength(
      brookBend.grassCells.length,
    );
    const harvestSpiralState = createInitialState(12345, "harvest-spiral");
    ...
    const forkedThicketState = createInitialState(12345, "forked-thicket");
    expect(forkedThicketState.targets.filter((target) => target.kind === "grass")).toHaveLength(
      forkedThicket.grassCells.length,
    );
    const switchbackOrchardState = createInitialState(12345, "switchback-orchard");
    expect(switchbackOrchardState.targets.filter((target) => target.kind === "grass")).toHaveLength(
      switchbackOrchard.grassCells.length,
    );
    const braidedMeadowState = createInitialState(12345, "braided-meadow");
    ...
    const twinGladeState = createInitialState(12345, "twin-glade");
    expect(twinGladeState.targets.filter((target) => target.kind === "grass")).toHaveLength(
      twinGlade.grassCells.length,
    );
    const frostRibbonsState = createInitialState(12345, "frost-ribbons");
    expect(frostRibbonsState.targets.filter((target) => target.kind === "grass")).toHaveLength(
      frostRibbons.grassCells.length,
    );
    const prismPrairieState = createInitialState(12345, "prism-prairie");
    ...
    const willowWeaveState = createInitialState(12345, "willow-weave");
    expect(willowWeaveState.targets.filter((target) => target.kind === "grass")).toHaveLength(
      willowWeave.grassCells.length,
    );
    const petalGateState = createInitialState(12345, "petal-gate");
    expect(petalGateState.targets.filter((target) => target.kind === "grass")).toHaveLength(
      petalGate.grassCells.length,
    );
    const sunsetSwitchbackState = createInitialState(12345, "sunset-switchback");
    ...
    const lagoonBraidState = createInitialState(12345, "lagoon-braid");
    expect(lagoonBraidState.targets.filter((target) => target.kind === "grass")).toHaveLength(
      lagoonBraid.grassCells.length,
    );
    const wildflowerNarrowsState = createInitialState(12345, "wildflower-narrows");
    expect(wildflowerNarrowsState.targets.filter((target) => target.kind === "grass")).toHaveLength(
      wildflowerNarrows.grassCells.length,
    );
    const berryBloomState = createInitialState(12345, "berry-bloom");
    expect(berryBloomState.targets.filter((target) => target.kind === "grass")).toHaveLength(
      berryBloom.grassCells.length,
    );
    const daisyDriftState = createInitialState(12345, "daisy-drift");
    expect(daisyDriftState.targets.filter((target) => target.kind === "grass")).toHaveLength(
      daisyDrift.grassCells.length,
    );
    ...
    expect(reedRun.grassCells.length).toBeGreaterThanOrEqual(reedRunState.objectives.grass.target);
    expect(orchardLoop.grassCells.length).toBeGreaterThanOrEqual(
      orchardLoopState.objectives.grass.target,
    );
    expect(brookBend.grassCells.length).toBeGreaterThanOrEqual(
      brookBendState.objectives.grass.target,
    );
    ...
    expect(forkedThicket.grassCells.length).toBeGreaterThanOrEqual(
      forkedThicketState.objectives.grass.target,
    );
    expect(switchbackOrchard.grassCells.length).toBeGreaterThanOrEqual(
      switchbackOrchardState.objectives.grass.target,
    );
    ...
    expect(twinGlade.grassCells.length).toBeGreaterThanOrEqual(
      twinGladeState.objectives.grass.target,
    );
    expect(frostRibbons.grassCells.length).toBeGreaterThanOrEqual(
      frostRibbonsState.objectives.grass.target,
    );
    ...
    expect(willowWeave.grassCells.length).toBeGreaterThanOrEqual(
      willowWeaveState.objectives.grass.target,
    );
    expect(petalGate.grassCells.length).toBeGreaterThanOrEqual(
      petalGateState.objectives.grass.target,
    );
    ...
    expect(lagoonBraid.grassCells.length).toBeGreaterThanOrEqual(
      lagoonBraidState.objectives.grass.target,
    );
    expect(wildflowerNarrows.grassCells.length).toBeGreaterThanOrEqual(
      wildflowerNarrowsState.objectives.grass.target,
    );
    expect(berryBloom.grassCells.length).toBeGreaterThanOrEqual(
      berryBloomState.objectives.grass.target,
    );
    expect(daisyDrift.grassCells.length).toBeGreaterThanOrEqual(
      daisyDriftState.objectives.grass.target,
    );
  ...
  it("keeps each authored arena visibly path-shaped instead of a full square", () => {
    ...
    expect(hasGrassCellNear(reedRun, -15, -5)).toBe(true);
    expect(hasGrassCellNear(reedRun, -9, 13)).toBe(true);
    expect(hasGrassCellNear(reedRun, 14, 5)).toBe(true);
    expect(hasGrassCellNear(reedRun, 10, -13)).toBe(true);
    expect(hasGrassCellNear(reedRun, -11, 3.2)).toBe(false);
    expect(hasGrassCellNear(reedRun, 7.4, 6.9)).toBe(false);
    expect(hasGrassCellNear(reedRun, 17, 17)).toBe(false);
    ...
    expect(hasGrassCellNear(brookBend, -16, -14)).toBe(true);
    expect(hasGrassCellNear(brookBend, 15, -2)).toBe(true);
    expect(hasGrassCellNear(brookBend, -14, 3)).toBe(true);
    expect(hasGrassCellNear(brookBend, 13, 12)).toBe(true);
    expect(hasGrassCellNear(brookBend, -2, -8)).toBe(false);
    expect(hasGrassCellNear(brookBend, 2, 13)).toBe(false);
    expect(hasGrassCellNear(brookBend, 17, 17)).toBe(false);

    expect(hasGrassCellNear(harvestSpiral, -17, -12)).toBe(true);
    expect(hasGrassCellNear(harvestSpiral, 17, -2)).toBe(true);
    expect(hasGrassCellNear(harvestSpiral, -2, 16)).toBe(true);
    expect(hasGrassCellNear(harvestSpiral, 2, 4)).toBe(true);
    ...
    expect(hasGrassCellNear(forkedThicket, 0, -18)).toBe(true);
    expect(hasGrassCellNear(forkedThicket, -16, -15)).toBe(true);
    expect(hasGrassCellNear(forkedThicket, 17, -10)).toBe(true);
    expect(hasGrassCellNear(forkedThicket, -15, 10)).toBe(true);
    expect(hasGrassCellNear(forkedThicket, 15, 10)).toBe(true);
    expect(hasGrassCellNear(forkedThicket, 0, 18)).toBe(true);
    expect(hasGrassCellNear(forkedThicket, -6, -1)).toBe(false);
    expect(hasGrassCellNear(forkedThicket, 6, -1)).toBe(false);
    expect(hasGrassCellNear(forkedThicket, 17, 17)).toBe(false);

    expect(hasGrassCellNear(switchbackOrchard, -17, -15)).toBe(true);
    expect(hasGrassCellNear(switchbackOrchard, 17, -10)).toBe(true);
    expect(hasGrassCellNear(switchbackOrchard, -14, -3)).toBe(true);
    expect(hasGrassCellNear(switchbackOrchard, 14, 4)).toBe(true);
    expect(hasGrassCellNear(switchbackOrchard, -14, 10)).toBe(true);
    expect(hasGrassCellNear(switchbackOrchard, 7, 18)).toBe(true);
    expect(hasGrassCellNear(switchbackOrchard, 0, -9)).toBe(false);
    expect(hasGrassCellNear(switchbackOrchard, 0, 8)).toBe(false);
    expect(hasGrassCellNear(switchbackOrchard, 17, 17)).toBe(false);

    expect(hasGrassCellNear(braidedMeadow, -17, -14)).toBe(true);
    expect(hasGrassCellNear(braidedMeadow, 16, -15)).toBe(true);
    expect(hasGrassCellNear(braidedMeadow, -16, -1)).toBe(true);
    expect(hasGrassCellNear(braidedMeadow, 16, 2)).toBe(true);
    expect(hasGrassCellNear(braidedMeadow, -16, 11)).toBe(true);
    expect(hasGrassCellNear(braidedMeadow, 17, 16)).toBe(true);
    expect(hasGrassCellNear(braidedMeadow, 0, -10)).toBe(false);
    expect(hasGrassCellNear(braidedMeadow, 0, 6)).toBe(false);
    ...
    expect(hasGrassCellNear(twinGlade, -10, -6)).toBe(true);
    expect(hasGrassCellNear(twinGlade, 10, -6)).toBe(true);
    expect(hasGrassCellNear(twinGlade, -11, 8, 1.2)).toBe(true);
    expect(hasGrassCellNear(twinGlade, 11, 8, 1.2)).toBe(true);
    expect(hasGrassCellNear(twinGlade, 0, -6)).toBe(false);
    expect(hasGrassCellNear(twinGlade, 0, 8)).toBe(false);
    expect(hasGrassCellNear(twinGlade, 17, 17)).toBe(false);

    expect(hasGrassCellNear(frostRibbons, -18, -14, 1.1)).toBe(true);
    expect(hasGrassCellNear(frostRibbons, 17, -10, 1.1)).toBe(true);
    expect(hasGrassCellNear(frostRibbons, -16, -1, 1.1)).toBe(true);
    expect(hasGrassCellNear(frostRibbons, 15, 6, 1.1)).toBe(true);
    expect(hasGrassCellNear(frostRibbons, -8, -7)).toBe(false);
    expect(hasGrassCellNear(frostRibbons, 8, 10)).toBe(false);
    expect(hasGrassCellNear(frostRibbons, 17, 17)).toBe(false);

    expect(hasGrassCellNear(prismPrairie, -16, -12)).toBe(true);
    expect(hasGrassCellNear(prismPrairie, 14, -15)).toBe(true);
    expect(hasGrassCellNear(prismPrairie, 18, 7)).toBe(true);
    expect(hasGrassCellNear(prismPrairie, -14, 10)).toBe(true);
    ...
    expect(hasGrassCellNear(willowWeave, -16, -15)).toBe(true);
    expect(hasGrassCellNear(willowWeave, 17, -10)).toBe(true);
    expect(hasGrassCellNear(willowWeave, -8.5, -7)).toBe(true);
    expect(hasGrassCellNear(willowWeave, 8, 9)).toBe(true);
    expect(hasGrassCellNear(willowWeave, 7, 18)).toBe(true);
    expect(hasGrassCellNear(willowWeave, 0, -12)).toBe(false);
    expect(hasGrassCellNear(willowWeave, 0, 5)).toBe(false);
    expect(hasGrassCellNear(willowWeave, 12, -1)).toBe(false);

    expect(hasGrassCellNear(petalGate, 0, -17)).toBe(true);
    expect(hasGrassCellNear(petalGate, -14, -10)).toBe(true);
    expect(hasGrassCellNear(petalGate, 14, -10)).toBe(true);
    expect(hasGrassCellNear(petalGate, -17, 8)).toBe(true);
    expect(hasGrassCellNear(petalGate, 0, 16, 1.1)).toBe(true);
    expect(hasGrassCellNear(petalGate, -8, -1)).toBe(false);
    expect(hasGrassCellNear(petalGate, 8, 13)).toBe(false);
    expect(hasGrassCellNear(petalGate, 0, -11)).toBe(false);

    expect(hasGrassCellNear(sunsetSwitchback, -17, -16)).toBe(true);
    expect(hasGrassCellNear(sunsetSwitchback, -4, -17)).toBe(true);
    expect(hasGrassCellNear(sunsetSwitchback, 16, -8)).toBe(true);
    expect(hasGrassCellNear(sunsetSwitchback, -17, 2)).toBe(true);
    expect(hasGrassCellNear(sunsetSwitchback, 17, 13)).toBe(true);
    expect(hasGrassCellNear(sunsetSwitchback, -17, 12)).toBe(true);
    expect(hasGrassCellNear(sunsetSwitchback, 0, -12)).toBe(false);
    expect(hasGrassCellNear(sunsetSwitchback, 9, -8)).toBe(false);
    ...
    expect(hasGrassCellNear(lagoonBraid, -17, -16)).toBe(true);
    expect(hasGrassCellNear(lagoonBraid, -6, -10)).toBe(true);
    expect(hasGrassCellNear(lagoonBraid, 8, -13)).toBe(true);
    expect(hasGrassCellNear(lagoonBraid, 17, -6)).toBe(true);
    expect(hasGrassCellNear(lagoonBraid, -17, 6)).toBe(true);
    expect(hasGrassCellNear(lagoonBraid, -6, 14)).toBe(true);
    expect(hasGrassCellNear(lagoonBraid, 17, 16)).toBe(true);
    expect(hasGrassCellNear(lagoonBraid, 17, -16)).toBe(true);
    expect(hasGrassCellNear(lagoonBraid, -8, -12)).toBe(true);
    expect(hasGrassCellNear(lagoonBraid, -3, 1)).toBe(true);
    expect(hasGrassCellNear(lagoonBraid, 17, 7)).toBe(true);
    expect(hasGrassCellNear(lagoonBraid, -17, 16)).toBe(true);
    expect(hasGrassCellNear(lagoonBraid, 0, -6)).toBe(false);
    expect(hasGrassCellNear(lagoonBraid, 0, 8)).toBe(false);
    expect(hasGrassCellNear(lagoonBraid, 10, -1)).toBe(false);
    expect(hasGrassCellNear(lagoonBraid, -10, 2)).toBe(false);
    expect(hasGrassCellNear(lagoonBraid, 10, 12)).toBe(false);
    expect(hasGrassCellNear(lagoonBraid, 0, 18)).toBe(false);

    expect(hasGrassCellNear(wildflowerNarrows, -17, -17)).toBe(true);
    expect(hasGrassCellNear(wildflowerNarrows, 3, -16)).toBe(true);
    expect(hasGrassCellNear(wildflowerNarrows, 17, -4)).toBe(true);
    expect(hasGrassCellNear(wildflowerNarrows, -17, 3)).toBe(true);
    expect(hasGrassCellNear(wildflowerNarrows, 15, 12)).toBe(true);
    expect(hasGrassCellNear(wildflowerNarrows, -8, 14)).toBe(true);
    expect(hasGrassCellNear(wildflowerNarrows, -1, -9)).toBe(false);
    expect(hasGrassCellNear(wildflowerNarrows, 8, -6)).toBe(false);
    expect(hasGrassCellNear(wildflowerNarrows, 9, 6)).toBe(false);
    expect(hasGrassCellNear(wildflowerNarrows, -3, 11)).toBe(false);
    expect(hasGrassCellNear(wildflowerNarrows, 17, 17)).toBe(false);

    expect(hasGrassCellNear(berryBloom, -14, -12)).toBe(true);
    expect(hasGrassCellNear(berryBloom, 10, -11)).toBe(true);
    expect(hasGrassCellNear(berryBloom, 14, -5)).toBe(true);
    expect(hasGrassCellNear(berryBloom, -9, -1)).toBe(true);
    expect(hasGrassCellNear(berryBloom, -10, 8)).toBe(true);
    ...
    expect(hasGrassCellNear(daisyDrift, 15, -10)).toBe(true);
    expect(hasGrassCellNear(daisyDrift, -13, 1)).toBe(true);
    expect(hasGrassCellNear(daisyDrift, 16, 7)).toBe(true);
    expect(hasGrassCellNear(daisyDrift, -5, 16)).toBe(true);
    expect(hasGrassCellNear(daisyDrift, -4, -10)).toBe(false);
    expect(hasGrassCellNear(daisyDrift, 7, -8)).toBe(false);
    expect(hasGrassCellNear(daisyDrift, -6, 5)).toBe(false);
    expect(hasGrassCellNear(daisyDrift, 1, 13)).toBe(false);
    expect(hasGrassCellNear(daisyDrift, 17, 17)).toBe(false);
  ...
  it("completes Harvest Spiral through its full mixed quota route", () => {
    const state = createInitialState(12345, "harvest-spiral");
    ...
    expect(state.mode).toBe("complete");
    expect(state.inventory).toEqual({ grass: 240, flowers: 260, fiber: 24, wood: 22 });
    expect(state.objectives.status).toBe("complete");
    expect(state.result).toMatchObject({
      status: "complete",
      timeLimitSeconds: 80,
      finalInventory: { grass: 240, flowers: 260, fiber: 24, wood: 22 },
    });
    expect(state.cutEvents.filter((event) => event.kind === "shrub")).toHaveLength(6);
    expect(state.cutEvents.filter((event) => event.kind === "sapling")).toHaveLength(5);
    expect(state.cutEvents.filter((event) => event.kind === "matureTree")).toHaveLength(2);
  });
...
function countCoolGrassVisuals(layout: MeadowLayout): number {
  return layout.grassVisuals.filter(
    (visual) => visual.height > 0 && visual.scaleX > 0 && visual.colorIndex >= 4,
  ).length;
}
...
function targetsForFlowerQuota(state: GameState, flowerQuota: number): TargetState[] {
  ...
  expect(remainingFlowers).toBeLessThanOrEqual(0);
  return targets;
}
