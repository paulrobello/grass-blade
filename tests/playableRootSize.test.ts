...
describe("contract navigation URLs", () => {
  ...
  it("opens the next authored contract while preserving diagnostics", () => {
    ...
    ).toBe("?seed=3258960835&debug=1&contract=berry-bloom");
    expect(
      contractNavigationSearch(
        1632666204,
    ...
    ).toBe("?seed=2445261230&debug=1&contract=timber-knot");
    expect(
      contractNavigationSearch(
        2445261230,
    ...
    expect(
      contractNavigationSearch(
        ...
        "?seed=2597112807&debug=1&contract=clear-every-patch",
      ),
    ).toBe("?seed=2597112807&debug=1");
  });
});
...
describe("contract chooser filters", () => {
  ...
  it("derives compact card badges from contract pacing, quotas, and route shape", () => {
    ...
    expect(contractCardBadges(contractById("berry-bloom"))).toEqual([
      "88s",
      "Fiber",
      "Hard",
      "Berry bloom",
    ]);
    expect(contractCardBadges(contractById("daisy-drift"))).toEqual([
      "76s",
      "Fiber",
      "Hard",
      "Daisy drift",
    ...
    expect(contractCardBadges(contractById("timber-knot"))).toEqual([
      "95s",
      "Wood",
      "Expert",
      "Timber knot",
  ...
  it("chooses a compact primary filter for each selected contract type", () => {
    ...
    expect(primaryContractFilterId(contractById("timber-knot"))).toBe("wood");
  ...
  it("matches contracts by broad chooser categories", () => {
    ...
    const timberKnot = contractById("timber-knot");
    ...
    expect(contractMatchesFilter(berryBloom, "timed")).toBe(true);
    ...
    expect(contractMatchesFilter(timberKnot, "wood")).toBe(true);
    expect(contractMatchesFilter(timberKnot, "timed")).toBe(false);
